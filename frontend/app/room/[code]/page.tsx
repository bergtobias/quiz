...
export default function RoomPage() {
  ...
  useEffect(() => {
    const newSocket = io("https://quiz.bergtobias.com", {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-md mx-auto">
            ...
            <CardContent className="p-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-800 mb-1">
                  {roomState.firstBuzzer.playerName}
                </div>
                <div className="text-sm text-yellow-600">
                  Team {roomState.firstBuzzer.team} buzzed first!
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          {Array.from({ length: roomState.teamCount }, (_, i) => i + 1).map(
            (teamNumber) => {
                ...
                <Card key={teamNumber}>
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-center gap-2">
                      <div
                        className={`w-4 h-4 rounded-full ${getTeamColor(
                          teamNumber
                        )}`}
                      ></div>
                      Team {teamNumber}
